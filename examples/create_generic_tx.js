...
async function getInput (addrDerivePath, redeemType, txid, scriptHex) {
  const scripts = {}
  let payment
  switch (redeemType) {
    case 'p2sh':
      payment = AddressService.generateDefaultBlockioAddress(btcBip32Priv, btcSecondaryPubKey, network)
      scripts.redeemScript = payment.redeem.output
      scripts.nonWitnessUtxo = Buffer.from(await getSochainTxHex(txid, networks.BITCOIN_TEST), 'hex')
      break
    case 'p2wsh':
      payment = AddressService.generateSubsequentBlockioAddress(btcBip32Priv, btcSecondaryPubKey, addrDerivePath, network)
      scripts.witnessScript = payment.redeem.output
      scripts.witnessUtxo = Buffer.from(scriptHex, 'hex')
      break
    case 'p2sh-p2wsh':
      payment = AddressService.generateP2wshBlockioAddress(btcBip32Priv, btcSecondaryPubKey, addrDerivePath, network)
      scripts.witnessScript = payment.redeem.redeem.output
      scripts.redeemScript = payment.redeem.output
      scripts.witnessUtxo = Buffer.from(scriptHex, 'hex')
      break
  }
